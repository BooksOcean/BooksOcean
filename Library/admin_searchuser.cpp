...
void admin_searchuser::DataBind() {
	...
	for (int i = currentPageBegin; i < currentPageEnd; i++) {
		...
		signalMapper->setMapping(btn2, DataTable[i].id);
		connect(signalMapper2, SIGNAL(mapped(int)), this, SLOT(OnBtnClicked(int)));
	}
}
...
void admin_searchuser::OnBtnClicked(int id)
{
